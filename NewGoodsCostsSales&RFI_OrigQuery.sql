...
---------------------------------------------------------------
declare @sDate date  -- start date is INclusive
declare @eDate date  -- end date is EXclusive
set @sDate = '6/1/21'
set @eDate = '7/1/21'
...
create table #Sales_prep(MonthBegining date
...
create table #Sales(MonthBegining date
					,LocationNo varchar(30)
					,DistSoldCost money
					,DistSoldVal money
					,FrlnSoldCost money
...
create table #Xfers_prep(MonthBegining date
						,LocationNo varchar(30)
						,XfrTy int
						,PrCat varchar(5)
						,Qty int
...
create table #Xfers(MonthBegining date
					,LocationNo varchar(30)
					,DistTshQty int
					,DistTshCost money
					,DistDmgQty int
...
select datefromparts(datepart(YY,sih.BusinessDate),datepart(MM,sih.BusinessDate),1)[MonthBegining]
	,loc.LocationNo
	,1[SldTy]
	...
	,cast(0 as money)[Fee]
from rHPB_Historical..SalesHeaderHistory_Recent shh with(nolock)
	inner join rHPB_Historical..SalesItemHistory_Recent sih with(nolock)
	...
	inner join ReportsData..ProductMaster pm with(nolock) on sih.ItemCode = pm.Itemcode 
	inner join ReportsData..ProductTypes pt with(nolock) on pm.ProductType = pt.ProductType 
where shh.Status = 'A'
	and shh.XactionType = 'S'
	and sih.BusinessDate >= @sDate
	and sih.BusinessDate < @eDate
	and pm.ProductType not in ('PGC ','EGC ')
	and pt.PTypeClass <> 'USED'
group by datefromparts(datepart(YY,sih.BusinessDate),datepart(MM,sih.BusinessDate),1)
	,loc.LocationNo
...
-- Shipped data--------
insert into #Sales_prep
select datefromparts(datepart(YY,om.OrderDate),datepart(MM,om.OrderDate),1)[MonthBegining]
...
-- Refunds data--------
insert into #Sales_prep
select datefromparts(datepart(YY,om.RefundDate),datepart(MM,om.RefundDate),1)[MonthBegining]
    ,isnull(od.LocationNo,fa.HPBLocationNo)[LocationNo]
	,-2[SldTy]
	,case when pm.ProductType = 'SCAN' then 'scan' when right(rtrim(pm.ProductType),1) <> 'F' then 'dist' else 'frln' end[PrCat]
...
-- Shipped data--------
insert into #Sales_prep
select datefromparts(datepart(YY,od.OrderDate),datepart(MM,od.OrderDate),1)[MonthBegining]
	,fa.HPBLocationNo[LocationNo]
	,3[SldTy]
	,case when pm.ProductType = 'SCAN' then 'scan' when right(rtrim(pm.ProductType),1) <> 'F' then 'dist' else 'frln' end[PrCat]
	,sum(od.Quantity)[Qty]
	,sum(pm.Cost)[Cost]
	,sum(od.ExtendedAmount)[Val]
	,sum(od.ShippingAmount)[Fee]
...
where od.OrderStatus not in ('canceled')
	...
	and left(od.SKU,1) = 'D'
	and od.Quantity > 0
group by datefromparts(datepart(YY,od.OrderDate),datepart(MM,od.OrderDate),1)
	,fa.HPBLocationNo
	,case when pm.ProductType = 'SCAN' then 'scan' when right(rtrim(pm.ProductType),1) <> 'F' then 'dist' else 'frln' end

-- Refunds data--------
insert into #Sales_prep
select datefromparts(datepart(YY,od.SiteLastModifiedDate),datepart(MM,od.SiteLastModifiedDate),1)[MonthBegining]
    ,fa.HPBLocationNo[LocationNo]
	,-3[SldTy]
	,case when pm.ProductType = 'SCAN' then 'scan' when right(rtrim(pm.ProductType),1) <> 'F' then 'dist' else 'frln' end[PrCat]
...
group by datefromparts(datepart(YY,od.SiteLastModifiedDate),datepart(MM,od.SiteLastModifiedDate),1)
    ,fa.HPBLocationNo
	,case when pm.ProductType = 'SCAN' then 'scan' when right(rtrim(pm.ProductType),1) <> 'F' then 'dist' else 'frln' end
...
------------------------------------------------------------------------------
insert into #Sales_prep
select cast(dateadd(MM,datediff(MM,0,sih.BusinessDate),0) as date)[MonthBegining]
    ,bw.LocationNo
	,0[SldTy]
	,case when pm.ProductType = 'SCAN' then 'scan' when right(rtrim(pm.ProductType),1) <> 'F' then 'dist' else 'frln' end[PrCat]
	,sum(sih.Quantity * case when sih.IsReturn = 'Y' then -1 else 1 end)[Qty]
...
group by cast(dateadd(MM,datediff(MM,0,sih.BusinessDate),0) as date)
    ,bw.LocationNo
	,case when pm.ProductType = 'SCAN' then 'scan' when right(rtrim(pm.ProductType),1) <> 'F' then 'dist' else 'frln' end
...
select datefromparts(datepart(YY,xh.CreateTime),datepart(MM,xh.CreateTime),1)[MonthBegining]
	,xh.LocationNo
	,TransferType[XfrTy]
	,case when pm.ProductType not like '%F' then 'dist' else 'frln' end[PrCat]
	,sum(xd.Quantity)[Qty]
	,sum(xd.DipsCost)[Cost]
from ReportsData..SipsTransferBinHeader xh with(nolock)
    inner join ReportsData..SipsTransferBinDetail xd with(nolock) on xh.TransferBinNo = xd.TransferBinNo 
    inner join ReportsData..ProductMaster pm with(nolock) on xd.DipsItemCode = pm.ItemCode 
	inner join ReportsData..ProductTypes pt with(nolock) on pm.ProductType = pt.ProductType 
where xh.StatusCode = 3
	and xd.StatusCode = 1
	and TransferType in (1,2,3,7)
	and xh.CreateTime >= @sDate
	and xh.CreateTime < @eDate
	-- and pm.ProductType not like '%F'
	and pm.ProductType not in ('PGC ','EGC ') 
	and pt.PTypeClass <> 'USED'
group by datefromparts(datepart(YY,xh.CreateTime),datepart(MM,xh.CreateTime),1)
	,xh.LocationNo
	,TransferType
...
---------------------------------------------------------------
;with Mos as(
	select distinct MonthBegining from #Sales
	union select distinct MonthBegining from #Xfers
)
, Locs as(
	select distinct LocationNo from #Sales
	union select distinct LocationNo from #Xfers
)
select m.MonthBegining
	,l.LocationNo
...
from Locs l cross join Mos m 
	left join #Sales sa on sa.MonthBegining = m.MonthBegining and sa.LocationNo = l.LocationNo
	left join #Xfers xr on xr.MonthBegining = m.MonthBegining and xr.LocationNo = l.LocationNo
order by m.MonthBegining
	,l.LocationNo
...
--Report Inputs - date range in month increments---------------
---------------------------------------------------------------
declare @eDate smalldatetime = cast(dateadd(DD,1-datepart(DD,getdate()),getdate()) as date)
declare @sDate smalldatetime = dateadd(MM,-1,@eDate)
...
;with Sales as(
	select datefromparts(datepart(YY,sih.BusinessDate),datepart(MM,sih.BusinessDate),1)[MonthBegining]
		,loc.LocationNo
		--Distribution-only product.....
		,sum(case when right(rtrim(pm.ProductType),1) <> 'F' then (sih.Quantity * pm.Cost * case when sih.IsReturn = 'Y' then -1 else 1 end) else 0 end)[DistSoldCost]
		,sum(case when right(rtrim(pm.ProductType),1) <> 'F' then sih.ExtendedAmt else 0 end)[DistSoldVal]
		--Frontline-only product....
		,sum(case when right(rtrim(pm.ProductType),1) = 'F' then (sih.Quantity * pm.Cost * case when sih.IsReturn = 'Y' then -1 else 1 end) else 0 end)[FrlnSoldCost]
		,sum(case when right(rtrim(pm.ProductType),1) = 'F' then sih.ExtendedAmt else 0 end)[FrlnSoldVal]
		--Scan-only data...
		,sum(case when pm.ProductType = 'SCAN' then sih.Quantity * case when sih.IsReturn = 'Y' then -1 else 1 end else 0 end)[ScanSoldQty]
		,sum(case when pm.ProductType = 'SCAN' then sih.Quantity * pm.Cost * case when sih.IsReturn = 'Y' then -1 else 1 end else 0 end)[ScanSoldCost]
		,sum(case when pm.ProductType = 'SCAN' then sih.ExtendedAmt else 0 end)[ScanSoldVal]
	from rHPB_Historical..SalesHeaderHistory_Recent shh with(nolock)
		inner join rHPB_Historical..SalesItemHistory_Recent sih with(nolock)
			on sih.LocationID = shh.LocationID
			and sih.BusinessDate = shh.BusinessDate
	...
	where shh.Status = 'A'
		and shh.XactionType = 'S'
		--Inexplicably, some stuff sells a $0.00. Still need to count the pieces. 
		--Not sure why I added this. Wasn't in the temp table version.
...
--Collect Removed From Inventory Data--------------------------
, Xfers as(
	select datefromparts(datepart(YY,xh.CreateTime),datepart(MM,xh.CreateTime),1)[MonthBegining]
		,xh.LocationNo
		,sum(case when TransferType = 1 then xd.Quantity else 0 end)[DistTshQty]
		,sum(case when TransferType = 1 then xd.DipsCost else 0 end)[DistTshCost]
		,sum(case when TransferType = 7 then xd.Quantity else 0 end)[DistDmgQty]
		,sum(case when TransferType = 7 then xd.DipsCost else 0 end)[DistDmgCost]
		,sum(case when TransferType = 2 then xd.Quantity else 0 end)[DistDntQty]
		,sum(case when TransferType = 2 then xd.DipsCost else 0 end)[DistDntCost]
		,sum(case when TransferType in (1,2,7) then xd.Quantity else 0 end)[TotDistRfiQty]
		,sum(case when TransferType in (1,2,7) then xd.DipsCost else 0 end)[TotDistRfiCost]
	from ReportsData..SipsTransferBinHeader xh with(nolock)
		inner join ReportsData..SipsTransferBinDetail xd with(nolock) on xh.TransferBinNo = xd.TransferBinNo 
		inner join ReportsData..ProductMaster pm with(nolock) on xd.DipsItemCode = pm.ItemCode 
		inner join ReportsData..ProductTypes pt with(nolock) on pm.ProductType = pt.ProductType 
...
--Report Output------------------------------------------------
select isnull(sa.MonthBegining,xr.MonthBegining)[MonthBegining]
	,isnull(sa.LocationNo,xr.LocationNo)[LocationNo]
	,isnull(sa.DistSoldCost,0)[DistSoldCost]
	,isnull(sa.DistSoldVal,0)[DistSoldVal]
	,isnull(sa.FrlnSoldCost,0)[FrlnSoldCost]
	,isnull(sa.FrlnSoldVal,0)[FrlnSoldVal]
	,isnull(sa.ScanSoldQty,0)[ScanSoldQty]
	,isnull(sa.ScanSoldCost,0)[ScanSoldCost]
	,isnull(sa.ScanSoldVal,0)[ScanSoldVal]
	,isnull(xr.DistTshQty,0)[DistTshQty]
	,isnull(xr.DistTshCost,0)[DistTshCost]
	,isnull(xr.DistDmgQty,0)[DistDmgQty]
	,isnull(xr.DistDmgCost,0)[DistDmgCost]
	...
	,isnull(xr.TotDistRfiQty,0)[TotDistRfiQty]
	,isnull(xr.TotDistRfiCost,0)[TotDistRfiCost]
from Sales sa full outer join Xfers xr 
	on sa.MonthBegining = xr.MonthBegining and sa.LocationNo = xr.LocationNo
order by isnull(sa.LocationNo,xr.LocationNo)
	,isnull(sa.MonthBegining,xr.MonthBegining)
